...
O fraco acoplamento entre as tecnologias demonstra a divisão estruturada entre as funcionalidades. Ou seja, enquanto a Vercel está lidando apenas com a renderização de páginas e compilação do Javascript por meio do Next.js, o servidor da Heroku está disponível apenas para consultas via API construída no Laravel, que pode também servir a outra aplicação, podendo esta ser mobile, por exemplo. E, seguindo esta distribuição vertical, conforme \citeonline{COULOURIS} há ganho de desempenho, melhorando-se os tempos de carregamentos das páginas.
